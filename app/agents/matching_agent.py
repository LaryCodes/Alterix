...
class MatchingAgent(BaseAgent):
        ...
        for candidate in candidates:
            ...
            if score >= self.match_threshold:
                matches.append({
                    ...
                    "score": score,
                    "match_type": "direct" if score > 0.8 else "potential",
    ...
    def _calculate_match_score(self, context: Dict[str, Any], candidate: Dict[str, Any]) -> float:
        """
        ...
        for skill in offered_skills:
            if skill.get("name", "").lower() == requested_skill.get("name", "").lower():
                ...
                skill_score = max(skill_score, level_match)
        
        # Trust score (30%)
        trust_score = candidate.get("trust_score", 50) / 100.0
        ...
        final_score = (skill_score * 0.5) + (trust_score * 0.3) + (reciprocal_score * 0.2)
        return final_score
